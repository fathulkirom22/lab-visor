...
async def get_list_category_app(
    db: SessionDep,
    sort_by: Annotated[
        Literal["id", "name", "icon", "created_at"], Query()
    ] = "created_at",
    ...
    data = db.exec(
        select(CategoryApp).order_by(sort_order).offset((page - 1) * size).limit(size)
    ).all()
    if direction == "desc":
        data = data[::-1]
...
async def post_shortcut_app(db: SessionDep, item: CategoryAppCreate):
    db_item = CategoryApp(**item.model_dump())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return BaseResponse(data=jsonable_encoder(db_item))
